...
SUBDIRS = ssl/ src/ src/DaveMQ third_party/dht #modules/
DEBUG = $(shell ls -l | grep ^d | awk '{if($$9 == "release") print $$9}')
ROOT_DIR = $(shell pwd)
BIN_DIR = $(ROOT_DIR)/bin

FLAG = -lpthread -lcurl -lzlog -lev -ljemalloc -DUSE_JEMALLOC -DJEMALLOC_NO_DEMANGLE -O3 -I $(ROOT_DIR)/include #-Wunused-result
